...
const CACHE_NAME  = 'sandy-brain-proto-v117';
...
async function runCheck(reason) {
  ...
  for (const r of rems) {
    if (!isDue(nowMin, r.time)) continue;
  ...
  for (const h of habits) {
    ...
    if (!isDue(nowMin, h.time)) continue;
    const ok = await fireOnce(
      'hab_' + h.id,
  ...
  try {
    ...
    for (const s of WLK) {
      ...
      if (!isDue(nowMin, s.ping)) continue;
      const ok = await fireOnce('walk_' + s.k, '\uD83D\uDEB6 10-min walk time',
